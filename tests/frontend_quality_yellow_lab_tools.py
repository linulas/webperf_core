...
def run_test(langCode, url, device='phone'):
    ...
    rating = (5.0 * (float(yellow_lab) / 100.0)) + 0.5
    ...
    review += _("TEXT_PAGE_DOM_COMPLEXITY").format(
        return_dict["domComplexity"])
    review += _("TEXT_PAGE_DOM_MANIPULATIONS").format(
        return_dict["domManipulations"])
    ...
    review += _("TEXT_PAGE_CSS_COMPLEXITY").format(
        return_dict["cssComplexity"])
    ...
    return (float("{0:.2f}".format(rating)), review, return_dict)
